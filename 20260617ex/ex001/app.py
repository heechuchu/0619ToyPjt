from flask import Flask
from flask import request 

app = Flask(__name__)

@app.route('/')

def home():
    return "<h1>Hello Falsk - 김가희</h1>"
# http://192.168.0.48:5000/hello
@app.route('/hello')
def hello():
    return "Hello Student!"
# http://192.168.0.48:5000/me
@app.route('/me')
def me():
    return '나는 flask 개발자 입니다.'

@app.route('/about')
def about():
    return '소개 페이지'

# http://192.168.0.48:5000/users/1
# 동적 url 처리(Path Parameter)
# int    정수
# string 문자열
# float  실수
# path   전체경로 (중간에 /가 있어도 문자열 끝까지 모두 받음)\

# 1. 경로변수(path variable)
@app.route('/users/<int:user_id>')
def get_user(user_id):
    return f'{user_id} 사용자 조회'

# string
# http://127.0.0.1:5000/users/gildong
@app.route('/users/<string:name>')
def get_name(name):
    return f'{name}님 요청'

# float
# http://127.0.0.1:5000/pi/3.14
@app.route('/pi/<float:pi>')
def get_pi(pi):
    return f'pi: {pi}'

# path
# http://127.0.0.1:5000/files/images/cat.jpg
@app.route('/files/<path:filepath>')
def files(filepath):
    return f'filepath: {filepath}'

# Get

# 2. 쿼리스트링 (query string)

# http://127.0.0.1:5000/search/weather>> 쿼리스트링 방식(query string)
# @app.route('/search/<string:keyword>')
# def search(keyword):
#     return f'keyword: {keyword}'

# request
# http://127.0.0.1:5000/search?keyword=weather&age=20
@app.route('/search')
def search():
    keyword = request.args.get("keyword")
    age = request.args.get('age')
    return f'keyword: {keyword}, age: {age}'

# post
@app.route("/login_form")
def login_form():
    '''
    str = ''
    str += '<form action="http://127.0.0.1:5000/login_form">'
    str += '    <input type="text" name="u_id" placeholder="input user ID">'
    str += '<br>'
    str += '    <input type="password" name="u_pw" placeholder="input user PW>'
    str += '<br>'
    str += '    <input type="submit" value="SINGUP">'
    str += '</form>'

    return str
    '''

    return '''
    <form action="http://127.0.0.1:5000/login_form">
        input type="text" name="u_id" placeholder="input user ID">
            <br>
        <input type="password" name="u_pw" placeholder="input user PW>
            <br>
        <input type="submit" value="SINGUP">
    </form>

'''


@app.route('/login_confirm', methods=["POST"])
def login_confirm():
    u_id = request.form.get('u_id')
    u_pw = request.form.get('u_pw')

    if u_id == 'gildong' and u_pw == '1234':
        return f'{u_id}님 로그인 성공'
    else:
        return f'{u_pw}님 로그인 실패'

if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0')